...
class ApiClient {
    ...
    async queryStrategy(handState) {
        const requiredFields = ['hand_id', 'table_id', 'street', 'hero_pos', 'effective_stack_bb', 'pot_bb', 'action_line'];
        
        // 校验必需字段
        const missingFields = requiredFields.filter(f => !(f in handState));
        if (missingFields.length > 0) {
            const error = new Error(`HandState 缺少必需字段: ${missingFields.join(', ')}`);
        ...
        try {
            ...
            // 校验响应数据中包含策略数据
            if (!data.data || !data.data.actions || !Array.isArray(data.data.actions)) {
